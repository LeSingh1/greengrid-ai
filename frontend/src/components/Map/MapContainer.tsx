    ...
    <div style={{ position: 'absolute', top: 164, right: 16, zIndex: 18, width: 252, background: 'var(--color-bg-sidebar)', border: '1px solid var(--color-border-subtle)', borderRadius: 8, boxShadow: '0 8px 28px rgba(0,0,0,0.45)', overflow: 'hidden' }}>
      ...
      <div style={{ padding: 10, display: 'grid', gap: 10 }}>
        ...
        <div style={{ borderTop: '1px solid var(--color-border-subtle)', paddingTop: 8, display: 'grid', gap: 5 }}>
          <div style={{ fontSize: 9, color: 'var(--color-text-muted)', lineHeight: 1.45 }}>Data sources: clean planning grid, simulated growth model, GreenGrid AI scoring engine</div>
          <div style={{ fontSize: 9, color: 'var(--color-text-muted)', lineHeight: 1.45 }}>GreenGrid AI is a decision support simulator, not a final planning authority.</div>
        </div>
      </div>
    </div>
  )
...
function infraIcon(item: InfrastructureItem) {
  ...
  return L.divIcon({
    className: 'greengrid-infra-icon',
...
function suggestionIcon(rank: number) {
  return L.divIcon({
    className: 'greengrid-suggestion-icon',
...
export function MapContainer() {
  ...
  const placeSuggestedInfrastructure = useCallback((suggestionId: string) => {
    ...
    notify('success', `Placed suggested ${suggestion.category.replace(/_/g, ' ')}. GreenGrid AI scored this as a high-confidence location.`, 2800)
        ...
        <div style={{ position: 'absolute', inset: 0, isolation: 'isolate', borderRadius: 10, overflow: 'hidden' }}>
          ...
          >
            <TileLayer
              ...
              subdomains="abcd"
              opacity={0.74}
              className="greengrid-soft-basemap"
              eventHandlers={{
