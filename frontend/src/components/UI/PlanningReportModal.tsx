...
function PlanningReport({ onClose }: { onClose: () => void }) {
  ...
  const pitchSummary =
    planning.cityId === 'fremon'
      ? 'GreenGrid AI analyzed Fremon under 35 percent projected growth, detected emergency, education, transit, and green space gaps, recommended a targeted AI infrastructure plan, and improved City Health from 61 to 82 while serving 74,000 projected residents.'
      : `GreenGrid AI analyzed ${cityName} under ${planning.growthPercent}% projected growth, detected emergency, education, transit, and green space gaps, recommended a targeted infrastructure plan, and improved City Health from ${chBefore} to ${chAfter} while serving ${typeof residents === 'number' ? residents.toLocaleString() : residents} residents.`
  ...
  const copyReport = () => {
    const body = document.getElementById('greengrid-report-body')?.innerText
  ...
  const downloadJson = () => {
    ...
    a.href = url
    a.download = `greengrid-${planning.cityId}-impact-report.json`
    a.click()
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div id="greengrid-report-body" className="p-5 space-y-4">
          <section>
            <h3 className="font-mono text-[10px] tracking-widest uppercase mb-2" style={{ color: 'var(--color-accent-cyan)' }}>
              Executive Summary
            </h3>
            <p className="text-sm leading-relaxed" style={{ color: 'var(--color-text-secondary)' }}>
              GreenGrid AI analyzed <strong>{cityName}</strong>, identified infrastructure gaps, recommended a targeted AI plan, and measured before-and-after improvement.
            </p>
